...
public class TestQRCodeDetection : MonoBehaviour
{
    ...
    private void HandleAddedMarker(ARMarker addedMarker)
    {
        ...
        // Log the position and rotation of the added marker
        LogMarkerTransform(addedMarker);
    ...
    {
        // Log the position and rotation of the updated marker
        LogMarkerTransform(updatedMarker);
    ...
    /// <summary>
    /// Logs the position and rotation of a given ARMarker.
    /// </summary>
    /// <param name="marker">The ARMarker whose transform information is to be logged.</param>
    private void LogMarkerTransform(ARMarker marker)
    {
        Vector3 position = marker.transform.position;
        Quaternion rotation = marker.transform.rotation;

        Debug.Log($"Marker Position: {position}, Marker Rotation: {rotation}");
    }
}
